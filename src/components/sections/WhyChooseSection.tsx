...
export const WhyChooseSection = () => {
  return (
    <section className="relative py-32 overflow-hidden">
      ...
      <div className="gradient-orb gradient-orb-orange w-[400px] h-[400px] top-0 left-0 opacity-15" />
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {differentiators.map((item, index) => (
              <FadeInView key={item.title} delay={index * 0.1}>
                <motion.div
                  className="p-6 rounded-2xl border border-border/30 bg-card/30 backdrop-blur-sm h-full"
        ...
        <div>
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold leading-tight">
              <AnimatedText text="Our Proven 4-Step" />
              <span className="block gradient-text-blue">
                <AnimatedText text="Process" delay={0.2} />
              </span>
            </h2>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {process.map((item, index) => (
              <ScaleInView key={item.step} delay={index * 0.1}>
                <motion.div
                  className="relative p-8 rounded-3xl bg-card border border-border/30 h-full"
                  whileHover={{ y: -8, scale: 1.02 }}
                >
                  {/* Step number */}
                  <div className={`text-7xl font-display font-bold bg-gradient-to-br ${item.color} bg-clip-text text-transparent opacity-20 absolute -top-2 -left-2`}>
                    {item.step}
                  </div>

                  <div className="relative z-10 pt-8">
                    <div className={`w-12 h-1 rounded-full bg-gradient-to-r ${item.color} mb-6`} />
